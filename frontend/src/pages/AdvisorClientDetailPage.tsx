...
import { ArrowLeft, ArrowRight, Boxes, FlaskConical, LoaderCircle, Send } from 'lucide-react';
    ...
    <div className="space-y-6">
      ...
      <section className="rounded-lg border border-ink/10 bg-white p-6 shadow-panel">
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <div className="grid gap-2 text-sm sm:grid-cols-5">
            <Badge label="Status" value={formatEnum(client.status ?? 'ACTIVE')} />
      ...
      <section className="rounded-lg border border-ink/10 bg-white p-5 shadow-panel">
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-3" aria-live="polite">
          ...
          {!widgetsLoading && !widgetsError && !assignedWidgets.length && (
            <div className="md:col-span-3"><EmptyState title="No widgets assigned yet." description="Assign the first widget to build this client's personalized dashboard." action={<Link className="text-sm font-semibold text-sage" to={`/advisor/widgets/configure?clientId=${client.id}`}>Browse widgets</Link>} /></div>
          )}
        </div>
      </section>
    </div>
...
function AssignedWidgetCard({ assignment, returnTo }: Readonly<{ assignment: DashboardAssignment; returnTo: string }>) {
  ...
  return (
    <article className="rounded-md border border-ink/10 p-4 transition hover:border-sage/40 hover:bg-sage/[0.035] dark:border-white/10">
      ...
      <div className="mt-3 flex flex-wrap gap-2"><Link to={configurationUrl} className="inline-flex items-center gap-1 text-xs font-semibold text-sage"><FlaskConical className="h-3.5 w-3.5" />Simulate</Link><Link to={configurationUrl} className="inline-flex items-center gap-1 text-xs font-semibold text-ink/55 hover:text-sage">Edit configuration <ArrowRight className="h-3.5 w-3.5" /></Link></div>
    </article>
  );
}
