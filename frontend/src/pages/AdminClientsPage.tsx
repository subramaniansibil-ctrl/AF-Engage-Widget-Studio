    ...
    <div className="space-y-5">
      ...
      {panel && (
        <section className="rounded-md border border-ink/10 bg-white/65 p-4 shadow-panel backdrop-blur-xl dark:border-white/10 dark:bg-white/5 sm:p-5">
          ...
          {panel === 'bulk' ? <BulkUpload onDone={() => setPanel(null)} /> : panel === 'view' && selected ? <ClientView client={selected} onEdit={() => openPanel('edit', selected)} /> : <ClientForm client={panel === 'edit' ? selected : null} onDone={() => setPanel(null)} />}
        </section>
      )}
      ...
      <section className="space-y-3">
        <div className="grid gap-2 rounded-md border border-ink/10 bg-white/55 p-3 backdrop-blur-xl dark:border-white/10 dark:bg-white/5 md:grid-cols-[minmax(240px,1fr)_160px_190px_auto]">
          ...
          <input aria-label="Filter by assigned advisor" className={inputClass} value={advisor} onChange={(event) => setAdvisor(event.target.value)} placeholder="Assigned advisor" />
...
function ClientForm({ client, onDone }: { client: AdminClient | null; onDone: () => void }) {
  const dispatch = useAppDispatch();
  const [form, setForm] = useState<ClientUpsertRequest>(() => client ? fromClient(client) : emptyForm);
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      if (client) await updateClient({ id: client.id, body: form }).unwrap(); else await createClient(form).unwrap();
...
function BulkUpload({ onDone }: { onDone: () => void }) {
  ...
  function downloadTemplate() {
    const url = URL.createObjectURL(new Blob([clientTemplateCsv()], { type: 'text/csv;charset=utf-8' }));
  ...
  async function readFile(file: File | undefined) {
    if (!file) return;
    const parsed = parseClientCsv(await file.text()); setRows(parsed.rows); setErrors(parsed.errors); setFileName(file.name);
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col gap-3 rounded-md border border-dashed border-ink/15 bg-white/35 p-4 dark:border-white/15 dark:bg-white/[0.03] sm:flex-row sm:items-center sm:justify-between">
        <div><p className="text-sm font-semibold">Upload a CSV file</p><p className="mt-1 text-xs text-ink/55 dark:text-white/55">Valid rows import independently; invalid rows remain blocked with precise errors.</p></div>
