...
function WorkspaceLayout({ controls, metrics, chart, chartTitle, chartDescription, mainResult, howWorks, insight, nextStep, progress }: WorkspaceLayoutProps) {
  return <div className="space-y-5 print:space-y-3">
    <section className="rounded-md border border-sage/20 bg-sage/[0.06] p-4 sm:p-5 print:hidden">
    ...
    <div className="grid items-start gap-5 xl:grid-cols-[390px_minmax(0,1fr)]">
      <section className="rounded-xl border border-ink/10 bg-white/90 p-5 shadow-[0_8px_24px_rgba(6,38,61,0.06)] dark:border-white/10 dark:bg-white/5 sm:p-6 print:hidden"><h3 className="text-xl font-extrabold tracking-[-0.015em]">Build your scenario</h3><p className="mt-1.5 text-sm leading-6 text-ink/55 dark:text-white/55">Start with the example values, then adjust one item at a time. Results update instantly.</p><div className="mt-6 space-y-5">{controls}</div></section>
      <div className="min-w-0 space-y-4 print:space-y-2">
        <motion.section layout aria-live="polite" className="rounded-xl border border-sage/25 bg-gradient-to-br from-sage/15 via-white to-sky/45 p-6 shadow-[0_10px_30px_rgba(0,167,111,0.08)] dark:via-ink dark:to-ink print:break-inside-avoid print:p-3 print:shadow-none print:bg-white"><p className="text-[10px] font-extrabold uppercase tracking-[0.12em] text-sage">Most important result</p><p className="mt-2 text-3xl font-extrabold tracking-[-0.035em] sm:text-4xl print:mt-1 print:text-2xl">{mainResult.value}</p><p className="mt-1 text-sm font-bold text-ink/70 dark:text-white/70">{mainResult.label}</p><p className="mt-2 text-sm leading-6 text-ink/55 dark:text-white/55 print:mt-1 print:text-xs print:leading-5">{mainResult.detail}</p></motion.section>
        <motion.div layout className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4 print:grid-cols-2 print:gap-2">{metrics}</motion.div>
        {progress !== undefined && <section aria-label="Sustainability score" className="rounded-xl border border-ink/10 bg-white/90 p-5 shadow-sm dark:border-white/10 dark:bg-white/5 print:break-inside-avoid"><div className="flex items-center justify-between gap-3"><div><p className="text-sm font-bold">Plan sustainability score</p><p className="mt-1 text-xs text-ink/50">Higher means the selected income is more likely to last for the full period.</p></div><strong className="text-2xl font-extrabold text-sage">{progress}/100</strong></div><div className="mt-4 h-2.5 overflow-hidden rounded-full bg-ink/8 dark:bg-white/10"><motion.div className="h-full rounded-full bg-gradient-to-r from-ocean to-sage" animate={{ width: `${progress}%` }} transition={{ duration: 0.35 }} /></div></section>}
        <motion.section layout className="rounded-xl border border-ink/10 bg-white/90 p-5 shadow-[0_8px_24px_rgba(6,38,61,0.06)] dark:border-white/10 dark:bg-white/5 print:break-inside-avoid print:p-3 print:shadow-none print:bg-white sm:p-6"><h3 className="text-base font-bold print:text-sm">{chartTitle}</h3><p className="mt-1 text-xs leading-5 text-ink/50 dark:text-white/50 print:mt-0.5 print:leading-4">{chartDescription}</p><div className="mt-5 h-72 min-w-0 sm:h-80 print:mt-2 print:h-60">{chart}</div></motion.section>
