...
export function SavedSimulationsPanel({ simulations, loading, activeSimulation, currentSnapshot, onOpen }: SavedSimulationsPanelProps) {
  ...
  async function rename(simulation: Simulation) {
    ...
    try {
      await updateSimulation({ id: simulation.id, name: renameValue.trim(), inputs: simulation.inputs, results: simulation.results, result: simulation.result }).unwrap();
      setRenameId(undefined);
      dispatch(addToast({ title: 'Simulation renamed', description: `Saved as ${renameValue.trim()}.`, variant: 'success' }));
    } catch { dispatch(addToast({ title: 'Rename failed', variant: 'error' })); }
  }

  async function updateActive() {
    if (!activeSimulation) return;
    try {
      const updated = await updateSimulation({ id: activeSimulation.id, name: activeSimulation.name, inputs: currentSnapshot.inputs, results: currentSnapshot.results, result: currentSnapshot.summary }).unwrap();
      onOpen(updated);
      dispatch(addToast({ title: 'Simulation updated', description: `${updated.name} now includes your latest values.`, variant: 'success' }));
    } catch { dispatch(addToast({ title: 'Update failed', variant: 'error' })); }
  }

  async function duplicate(simulation: Simulation) {
    try {
      await duplicateSimulation({ id: simulation.id }).unwrap();
      dispatch(addToast({ title: 'Simulation duplicated', description: `${simulation.name} Copy is ready.`, variant: 'success' }));
    } catch { dispatch(addToast({ title: 'Duplicate failed', variant: 'error' })); }
  }
  ...
  async function remove(id: string) {
    ...
    } catch { dispatch(addToast({ title: 'Delete failed', variant: 'error' })); }
  }
  ...
  return <section className="space-y-4">
    <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-end"><div><p className="text-sm font-semibold text-sage">Saved simulations</p><h3 className="mt-1 text-xl font-bold">Your planning scenarios</h3><p className="mt-1 text-sm text-ink/55 dark:text-white/55">Reopen, refine, duplicate, or compare previous ideas.</p></div>{activeSimulation && <Button onClick={updateActive} disabled={updating}><Save className="h-4 w-4" />{updating ? 'Updating…' : `Update ${activeSimulation.name}`}</Button>}</div>
    {loading ? <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{Array.from({ length: 3 }).map((_, index) => <Skeleton key={index} className="h-48" />)}</div> : !simulations.length ? <EmptyState title="No saved simulations" description="Adjust the widget, then save your first named scenario." /> : <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{simulations.map((simulation) => <article key={simulation.id} className={['rounded-md border bg-white/60 p-4 shadow-sm dark:bg-white/5', activeSimulation?.id === simulation.id ? 'border-sage ring-2 ring-sage/10' : 'border-ink/10 dark:border-white/10'].join(' ')}>
      ...
      <p className="mt-3 line-clamp-2 min-h-10 text-sm leading-5 text-ink/60 dark:text-white/60">{simulation.result}</p><p className="mt-3 text-xs text-ink/40 dark:text-white/40">Modified {formatDate(simulation.updatedAt || simulation.createdAt)}</p>
      {renameId === simulation.id ? <div className="mt-3 flex gap-2"><input aria-label="Simulation name" autoFocus value={renameValue} onChange={(event) => setRenameValue(event.target.value)} className="min-h-9 min-w-0 flex-1 rounded-md border border-ink/10 px-2 text-sm dark:border-white/10 dark:bg-ink" /><Button onClick={() => rename(simulation)} disabled={updating}>Save</Button></div> : <div className="mt-4 flex flex-wrap gap-1"><IconButton label="Continue editing" onClick={() => onOpen(simulation)}><Play className="h-4 w-4" /></IconButton><IconButton label="Rename simulation" onClick={() => { setRenameId(simulation.id); setRenameValue(simulation.name); }}><Pencil className="h-4 w-4" /></IconButton><IconButton label="Duplicate simulation" onClick={() => duplicate(simulation)} disabled={duplicating}><Copy className="h-4 w-4" /></IconButton><IconButton label="Delete simulation" onClick={() => setDeleteId(simulation.id)}><Trash2 className="h-4 w-4" /></IconButton></div>}
